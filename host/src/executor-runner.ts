...
type ExecutionLogLevel = 'log' | 'info' | 'warn' | 'error'
...
async function execute(code: string): Promise<ExecutionEnvelope> {
...
function printEnvelope(envelope: ExecutionEnvelope): void {
  process.stdout.write(JSON.stringify(envelope))
}

function buildBootstrapFailure(code: string, message: string): ExecutionEnvelope {
  return {
    ok: false,
    result: null,
    logs: [],
...
async function main(): Promise<void> {
  ...
  try {
    payload = JSON.parse(rawInput) as { code?: unknown }
  } catch (error) {
